...
async function checkBackend(){
  ...
  const url = localStorage.getItem('backend_url') || 'https://kind-streets-jump.loca.lt';
  try{
...
setTimeout(()=>{ checkBackend(); $('#backendStatus').onclick=()=>{
  const cur=localStorage.getItem('backend_url')||'https://kind-streets-jump.loca.lt';
  const v=prompt('请输入后端代理地址：\n(自动公网隧道, https://xxx.loca.lt = 公网隧道)', cur);
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
      ...
      if(pl.length){
        ...
        setStep(2,'active');
        setBar(60);
        const backendUrl = localStorage.getItem('backend_url') || 'https://kind-streets-jump.loca.lt';
